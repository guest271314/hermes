...
std::unique_ptr<hbc::BytecodeModule> compileLazyFunction(
  ...
  auto bgo = BytecodeGenerationOptions::defaults();
  bgo.format = lazyData->context->getLazyDumpTarget();

  if (bgo.format > OutputFormatKind::DumpIR &&
      bgo.format < OutputFormatKind::DumpBytecode) {
    llvm::outs() << "**** Lazy dump ****\n\n";
  }

  auto bytecodeModule =
      hbc::generateBytecodeModule(&M, lexicalRoot, entryPoint, bgo);
...
void CodeBlock::lazyCompileImpl(Runtime *runtime) {
  ...
  auto bcProv = hbc::BCProviderFromSrc::createBCProviderFromSrc(
      compileLazyFunction(func->getLazyCompilationData()));

#ifndef NDEBUG
  if (func->getLazyCompilationData()->context->getLazyDumpTarget() ==
      OutputFormatKind::DumpBytecode) {
    llvm::outs() << "**** Lazy dump ****\n\n";

    // This is ugly. We need to create a shared_ptr to the provider, but then
    // we need a unique_ptr<> again. Since there is no easy way to transfer from
    // shared_ptr to unique_ptr, we create a shared_ptr with a dummy deleter.
    std::shared_ptr<hbc::BCProvider> shrBCProv{bcProv.get(),
                                               [](hbc::BCProviderFromSrc *) {}};
    hbc::BytecodeDisassembler disassembler(std::move(shrBCProv));
    disassembler.setOptions(hbc::DisassemblyOptions::Pretty);
    disassembler.disassemble(llvm::outs());
  }
#endif

  runtimeModule_->initializeLazyMayAllocate(std::move(bcProv));
